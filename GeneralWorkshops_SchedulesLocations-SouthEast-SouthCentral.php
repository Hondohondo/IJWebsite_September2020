<!DOCTYPE html>
<html lang="en">
    <?php include "headhtml.php";?><head>
        <title>Progoff Intensive Journal Program | For Self-Development</title>
        <meta charset="utf-8" />
        <meta name="description" content />
        <meta property="og:title" content="Progoff Intensive Journal Program | For Self-Development" />
        <meta property="og:description" content />
        <meta property="og:image" content />
        <meta property="og:type" content="website" />
        <meta
            property="og:url"
            content="https://www.intensivejournal.org/GeneralWorkshops_SchedulesLocations-SouthEast-SouthCentral.php"
        />
    </head>
    <body>
        <?php include "phone-nav.php";?>
        <div class="expand">
            <?php include "navigation.php";?>

            <div class="col-3-of-3">
                <div id="content" class="cms-editable">
                    <section class="section-inside">
                        <div class="section-inside-header">
                            <h5 class="heading-five">General Workshops</h5>
                            <!-- <h1 class="heading-one hr-overview">Schedules & Locations</h1> -->
                            <div class="inside-nav-container">
                                <hr class="work-hr" />
                                <ul class="inside-nav">
                                    <!-- <span class="vert">&vert;</span> -->
                                    <li>
                                        <a href="GeneralWorkshops_SchedulesLocations-SouthEast.php"
                                            >← Southeastern Region</a
                                        >
                                    </li>
                                    <!-- <span class="vert">&vert;</span> -->
                                </ul>
                                <hr class="work-hr" />
                            </div>
                        </div>
                        <div class="banner">
                            <div id="banner-img-32" class="banner-img"> </div>
                        </div>
                        <div id="my-content" class="section-inside-content">
                            <h1><em>Intensive Journal</em> Workshops - Southcentral Region</h1>
                            <!-- <br><br> -->
                            <h6>Winter - Summer, 2021</h6>
                            <p class="reg-flyer">
                                Please have a copy of our
                                <a class="page-link" href="southcentral-3p-1219.pdf" target="_blank" rel="noopener"
                                    >Southcentral regional flyer   - not ready</a
                                >
                            </p>
                            <h5>
                                <a id="new-orleans"></a>New Orleans, Louisiana:  Part 1 (Life Context) on June 25-26, 
                                2021 - online<br />                                                               New
                                Dates      
                            </h5>
                            <table class="table-credits table-praises" style="width: 79.3411%">
                                <tbody>
                                    <tr>
                                        <th style="width: 31.9058%">Workshops</th>
                                        <th style="width: 50.5361%">Schedule</th>
                                        <th style="width: 22.2416%">Tuition<sup>*</sup></th>
                                    </tr>
                                    <tr>
                                        <td style="width: 31.9058%">Life Context</td>
                                        <td style="width: 50.5361%">
                                            06/25: (1:30 -4:30 pm; 6-9 pm) <br />06/26: (9 am-12 pm; 1:30-4:30 pm)
                                        </td>
                                        <td style="width: 22.2416%">$ 225</td>
                                    </tr>
                                </tbody>
                            </table>
                            <h5><!-- <br><br> --></h5>
                            <p>*  Includes <em>Intensive Journal</em> workbook.</p>
                            <table class="table-credits table-praises">
                                <tbody>
                                    <tr>
                                        <th>Location</th>
                                        <td>Workshops held online through Zoom. </td>
                                    </tr>
                                    <tr>
                                        <th>Leader</th>
                                        <td>
                                            <strong><strong class>Dave Arbogast</strong> r</strong>ecently retired as an
                                            English Curriculum and Digital Resources Specialist for the Loudoun County
                                            Public School District in Northern Virginia.  Previously, he was a high
                                            school English teacher and a cross country coach. Dave is affiliated with
                                            the National Writing Project and is active in 12-Step recovery programs in
                                            Northern Virginia.  Dave has led <i class>Intensive Journal </i>workshops in
                                            the Mid-Atlantic region for six years.<strong><br /></strong
                                            ><strong><br /></strong>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Registration</th>
                                        <td>
                                            Call <strong> 330-998-6000 or 800-221-5844 </strong> or write to
                                            <strong>Dialogue House</strong> , 2245 E Enterprise Pkwy - #B1, Twinsburg,
                                            OH  44087-2348. Email: info@intensivejournal.org Credit cards accepted. Make
                                            checks payable to Dialogue House.  Minimum non-refundable deposit:  $100. 
                                            Full payment required at least two weeks before the workshop.
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <h5> </h5>
                            <h5>  </h5>
                            <h5>
                                <a id="dallas"></a>Dallas-Houston-Austin, Texas: 3-Part Series on October 16-23, 2021 -
                                online<br />                                                                           
                                new dates
                            </h5>
                            <table class="table-credits table-praises">
                                <tbody>
                                    <tr>
                                        <th>Workshops</th>
                                        <th>Schedule</th>
                                        <th>Tuition<sup>*</sup></th>
                                    </tr>
                                    <tr>
                                        <td>Life Context</td>
                                        <td>10/16: (9 am-12:30 pm; 2-5 pm) <br />10/17: (9 am-12 pm; 1:30-4:30 pm)</td>
                                        <td>$ 225</td>
                                    </tr>
                                    <tr>
                                        <td>Depth Contact</td>
                                        <td>10/19: (9 am-12:30 pm; 2-5 pm) <br />10/20: (9 am-12 pm; 1:30-4:30 pm)</td>
                                        <td>$ 210</td>
                                    </tr>
                                    <tr>
                                        <td>Life Integration</td>
                                        <td>10/22: (9 am-12:30 pm; 2-5 pm) <br />10/23: (9 am-12 pm; 1:30-4:30 pm)</td>
                                        <td>$ 210</td>
                                    </tr>
                                    <tr>
                                        <td>All three workshops</td>
                                        <td> </td>
                                        <td>$ 625</td>
                                    </tr>
                                </tbody>
                            </table>
                            <!-- <br><br> -->
                            <p>
                                *  Tuition only.   Workshop not held on April 26 and 29.   Life Context workshop
                                includes <em>Intensive Journal</em> workbook.
                            </p>
                            <table class="table-credits table-praises">
                                <tbody>
                                    <tr>
                                        <th>Location</th>
                                        <td>Workshops held online through Zoom. </td>
                                    </tr>
                                    <tr>
                                        <th>Leader</th>
                                        <td>
                                            <strong><strong class>Dave Arbogast</strong> r</strong>ecently retired as an
                                            English Curriculum and Digital Resources Specialist for the Loudoun County
                                            Public School District in Northern Virginia.  Previously, he was a high
                                            school English teacher and a cross country coach. Dave is affiliated with
                                            the National Writing Project and is active in 12-Step recovery programs in
                                            Northern Virginia.  Dave has led <i class>Intensive Journal </i>workshops in
                                            the Mid-Atlantic region for six years.<strong><br /></strong
                                            ><strong><br /></strong>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Registration</th>
                                        <td>
                                            Call <strong> 330-998-6000 or 800-221-5844 </strong> or write to
                                            <strong>Dialogue House</strong> , 2245 E Enterprise Pkwy - #B1, Twinsburg,
                                            OH  44087-2348. Email: info@intensivejournal.org Credit cards accepted. Make
                                            checks payable to Dialogue House.  Minimum non-refundable deposit:  $100. 
                                            Full payment required at least two weeks before the workshop.
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            <br /><br /><br /><!-- <table class="table-credits table-praises table-schedules"> -->
                            <!-- <hr class="trademark-line u-margin-top-small"> -->
                        </div>
                    </section>
                </div>

                <?php include "footer.php";?>
            </div>
        </div>

        <script src="https://unpkg.com/ionicons@4.5.10-0/dist/ionicons.js"></script>
    </body>
</html>
